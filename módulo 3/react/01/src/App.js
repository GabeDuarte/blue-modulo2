...
export default class App extends React.Component {
  ...
  onSubmit= (e) =>{
    ...
    const {series, editando, indexEditando, nome,  imagem_url, qtd_temp, ano_lancamento } = this.state;
    ...
    if (editando){
      const seriesAtualizadas = series.map((serie, index) => {
        if (indexEditando === index){
          ...
          serie.imagem_url = imagem_url;
          serie.qtd_temp = qtd_temp;
  ...
  render(){
    const {series, nome, imagem_url, ano_lancamento, qtd_temp, editando, indexEditando} = this.state;
    return (
      <div className="container">
        <main className="main">
          ...
          <hr/>
          <h2>
            {editando?`Editando:${series[indexEditando].nomeSerie}`:"Cadastrar uma nova serie"}
          </h2>
          <form onSubmit={this.onSubmit}>
            <input placeholder = 'nome' value={nome} onChange={(e)=>{
              this.setState({
                nome: e.target.value
              });
            }}/>
            <br></br>
            <input placeholder = 'URL da imagem' value={imagem_url} onChange={(e)=>{
              this.setState({
                imagem_url: e.target.value
              });
            }}/>
            <br></br>
            <input placeholder = 'Ano de lançamento' value={ano_lancamento} onChange={(e)=>{
              this.setState({
                ano_lancamento: e.target.value
              });
            }}/>
            <br></br>
            <input placeholder = 'Quantidade de temporadas' value={qtd_temp} onChange={(e)=>{
              this.setState({
                qtd_temp: e.target.value
              });
            }}/>
            <br></br>
